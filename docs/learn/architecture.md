...
- **`@holotope/experiment`** — a document boundary for describing a
  source–model–representation experience and its headless runtime. Validation
  and identity never construct runtime objects; compilation receives explicit
  caller capabilities rather than consulting a global registry, and is
  all-or-nothing. Parameters read and write live targets, actions and
  observations remain bounded declarations, and snapshots/traces preserve
  exact CPU replay. Pane descriptors are retained for renderer adapters but
  never become headless registry objects.
...
by mesh size or a mode string, and must index the same obstacle and group
objects the family does. It changes which pairs are asked, never what a
retained pair means: node bounds union their descendants so a separated node
proves rejection for all of them, traversal order is restored to persistent
obstacle-cell order before candidates are public, and the exact barrier plus
paired prefix filter remain authoritative behind it. Because it caches bounds
once, it snapshots the coordinates it indexed and refuses a moved obstacle by
name rather than rebuilding silently — stale acceleration that looks plausible
is the failure worth being loud about. Worst-case work stays linear and its
diagnostics report that rather than averaging it away.

That same authored world can also be advanced by the incremental-potential
transaction instead, through one world-scoped step that derives dimension,
particle order, gravity, and the conservative provider registry from the world
and delegates once. The two paths are alternatives over one physical interval,
never a sequence: projected XPBD applies registered constraints, responses,
and guards, while the optimization path cannot represent any of the three. Its
selection stage therefore names the first such registration as a configuration
error, along with any registered force provider that is not conservative,
rather than silently solving the representable subset. Step filters stay
caller-supplied because the base world owns no filter registry, and the
...
typed constitutive-law seam and source-identified generic family compiler copy
authored rest positions, evaluate one law per selected simplex cell, assemble
shared-particle forces, and implement the RN force-provider contract without
making the visible source or an edge network authoritative. Named StVK and
Neo-Hookean wrappers preserve discoverability while sharing the same lineage,
ownership, and accumulation implementation. A separately registered
accepted-state guard can reevaluate that same family after each completed RN
substep, retain admissibility evidence, and reject orientation change or a
configured minimum-measure violation. `XpbdWorldN` rolls a rejection back and
can retry the same outer interval with bounded deterministic subdivision.
This is a sampled endpoint policy, not continuous orientation CCD or an
inversion barrier. Implicit stiffness handling and accelerated implementations
also remain separate policies.
...
The same compilation boundary extends to local material coordinates.
Simplex-family compilation retains a structural id per authored simplex while
sharing the network's particles. Full-dimensional cuboid compilation first
uses the deterministic Kuhn decomposition, then retains both the authored
parent-cuboid id and the local axis permutation for every signed simplex
constraint. Generated simulation elements therefore remain traceable to source
...
10. ◐ `@holotope/physics`: convex R4 mass properties, ballistic and prescribed-kinematic bodies, scene synchronization, GJK with coherent caches, dimension-independent swept broadphase, simplex metric deformation with typed StVK and compressible Neo-Hookean references plus provenance-preserving generic constitutive family assembly and accepted-state guards, candidate-state and mass-weighted incremental objectives with packed Armijo, bounded first-order and matrix-free Newton direction policies, exact analytic provider curvature, opt-in dense provider-local and exact provider-block PSD references, source-simplex block decomposition, and fixed-coordinate compiled-curvature reuse across Krylov iterations, atomic result-application references, ordered admissible-step filters with an exact RN point–static-plane specialization and a source-retained finite point–simplex barrier with conservative prefix certification, source-indexed paired point–plane barrier/filter families, and an exhaustive source-retained dynamic-point/static-simplex candidate family with conservative swept-AABB rejection plus an opt-in immutable static-obstacle AABB hierarchy with deterministic construction, restored source order, and stale-source refusal, world-scoped nonlinear advancement that derives its authoritative inputs from one registry and refuses unrepresentable world features by name, intrinsic simplex mass lumping, topology-neutral source-particle bindings, per-substep RN force providers, bounded transactional adaptive subdivision, and projected XPBD scalar relations including exact RN particle–hyperplane contact plus unsigned intrinsic and signed full-dimensional simplex coordinates, ordered post-projection RN velocity policies with isotropic particle–plane Coulomb friction and timestep-invariant damping, an RN point world and provenance-preserving `CellComplex` distance, simplex, and cuboid-volume compilers, conservative linear casts, explicit constant-generator R4 trajectories and conservative rigid casts, shared dynamic/kinematic pose plans, opt-in rotational R4 event stepping, bounded general R4 EPA penetration, persistent polytope manifolds, analytic mixed contacts, coupled three-ball friction, deterministic mixed-shape orchestration, point/distance policies, branch-aware SO(4) coordinates, common small equality and one-bounded blocks, direction preservation with its SO(3) stabilizer, planar SO(2) coordinates with torque-limited motors and continuous-angle guardians, and six-row fixed-relative-frame orientation joints; maintained moving--moving/self-contact active sets, moving-obstacle hierarchy refit, cached sparse/global modified Newton, bending, distance servos, rolling resistance, and sleeping pending
11. ◐ `@holotope/experiment`: bounded schema-v0 intake, semantic validation,
    canonical SHA-256 document identity, dependency planning, explicit
    caller-capability source/model/representation compilation, derived
    lineage witnesses, live parameter reads and writes, bounded actions,
    stamped observations, exact CPU snapshots, traces, replay, and
    renderer-deferred pane descriptors; reusable presentation workbench and
    broader descriptor vocabularies pending
